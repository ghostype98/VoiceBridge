...
# 从 config/config.yaml 读取端口（唯一配置来源）
MAIN_PORT=$(python3 -c "
import yaml, os
p = '$PROJECT_DIR'
cfg = os.path.join(p, 'config', 'config.yaml')
...
if os.path.isfile(cfg):
    with open(cfg) as f:
        c = yaml.safe_load(f) or {}
    vs = c.get('voice_streaming', {}) or {}
    print(vs.get('websocket', {}).get('port', 8003))
else:
    print(8003)
" 2>/dev/null) || WS_PORT=8003
log "从 config/config.yaml 读取端口: 主服务=$MAIN_PORT, WebSocket=$WS_PORT"
...
if ps -p $SERVICE_PID > /dev/null 2>&1; then
    log "✓ 服务启动成功！"
    log "  - 主服务: http://0.0.0.0:$MAIN_PORT"
    log "  - WebSocket: ws://0.0.0.0:$WS_PORT"
    log "  - 前端页面: http://localhost:$MAIN_PORT/login"
    log "  - API文档: http://localhost:$MAIN_PORT/docs"
    log "  - 日志文件: $LOG_FILE"
    log "  - PID文件: $PID_FILE"
    log ""
