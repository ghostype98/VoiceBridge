...
from voice_interview_streaming.websocket_server import VoiceInterviewWebSocketServer
...
async def create_session(request: CreateSessionRequest):
    ...
    try:
        ...
        INSERT INTO voice_interview_sessions (
            session_id, invitation_id, question_id, status, start_time
        ) VALUES (%s, %s, %s, %s, %s)
        """
        if db_manager.db_type != 'postgresql':
...
async def get_session_status(session_id: str):
    ...
    try:
        ...
        # 获取会话信息
        session_sql = """
        SELECT session_id, question_id, start_time, status
        ...
        session = SessionInfo(
            ...
            start_time=session_data['start_time'].isoformat() if hasattr(session_data['start_time'], 'isoformat') else str(session_data['start_time']),
            status=session_data['status']
        )

        # 获取评价历史
        evaluations_sql = """
        SELECT id, session_id, question_id, answer_text, score, reason,
               evaluation_details, need_follow_up, follow_up_question, created_at
        ...
        evaluations_data = db_manager.fetch_all(evaluations_sql, (session_id,))
...
async def get_interview_questions(invitation_id: str):
    ...
    try:
        ...
        questions = db_manager.fetch_all(questions_sql, (invitation_id,))
...
def get_llm() -> Optional[LangChainWrapper]:
    """获取LLM包装器"""
    from api.utils import get_llm
    return get_llm()
