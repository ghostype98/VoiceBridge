...
from voice_interview_streaming.database_setup import create_voice_interview_tables
class ConfigWrapper:
...
class VoiceInterviewServiceLauncher:
    ...
    def _validate_config(self) -> bool:
        ...
        try:
            # 获取语音面试配置
            voice_config = self.config.get('voice_interview_streaming', {})
            ...
            required_configs = [
                ...
                'asr.appkey',
                'asr.token',
                'evaluation.enable_real_time_scoring'
            ]
            ...
            for config_path in required_configs:
                ...
                for key in keys:
                    if isinstance(value, dict) and key in value:
                        ...
                        logger.error(f"配置项缺失: {config_path}")
                        return False

                if value is None or value == '':
                    logger.error(f"配置项为空: {config_path}")
                    return False
